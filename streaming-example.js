...
async function runTest(apiKey, customerId, uri, filePath) {
    ...
    ws.onopen = async () => {
        ...
        console.error(`Channels = ${channelData.length}, Sample Rate = ${sampleRate} Hz, Sample width = 2 bytes`);
...
(async () => {
    ...
    if (filePath) {
        ...
        console.log("Please pass the file path as an argument to stream a prerecorded audio file");
        console.log("How to run the script:");
        console.log("node streaming_client_demo.js -f streaming_demo.wav");
    }
})();
